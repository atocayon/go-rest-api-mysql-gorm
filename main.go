...
func initDB()  {
	var err error
	dataSourceName := "root:admin@tcp(localhost:3306)/?parseTime=True"
	...
	// Create database. This is a one-time step.
	// Comment out if running multiple times - You may see an error otherwise
	db.Exec("CREATE DATABASE orders_db")
	db.Exec("USE orders_db")

	// Migration to create tables for Orders and Item schema
